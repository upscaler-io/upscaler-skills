...
- Adding or renaming a skill changes every distributable package. Run `python3 scripts/build_bundle.py && python3 scripts/build_chatgpt_skills.py && python3 scripts/build_editor_bundles.py` to confirm all three packagers still succeed; `.github/workflows/package.yml` runs the same three on every push and PR.
