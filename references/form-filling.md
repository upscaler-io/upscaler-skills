...
| Values shape | flat: `values.<key>` | nested per task: `values.<taskDefinitionId>.<key>` |
| Create payload | `{"values": {...}}` | `{"title": ...}` only, then draft each task separately |
| Update path | `entry update --entry-id i_*` / `upscaler_manage_entry(operation:"update", entry_id)` | per task: `entry save-draft --task-id t_* --note "…"` / `upscaler_save_task_draft({record_id, task_id, note, values})` |
...
**Agent writes never finalize (HITL contract).** Exactly one mutation was removed from the agent surface: `completeTask`. Everything else still works and is simply *rerouted* into a draft. A record `create` may carry `taskValues` keyed by `td_*`, and a record `update` may carry a `task_id`; both are dispatched to `saveTaskDraft`, drafting each task in record order and synthesizing a reviewer note when you omit one. Every such write leaves the task in DRAFT for a human to complete in up-app. Register entries follow the same spirit: a create lands as DRAFT, and an update to a COMPLETED entry is stashed as a pending revision instead of changing live values. Never report a task, record, or entry as completed, closed, or live after a write.

Drafts validate loosely on purpose: values are checked for type and shape only, and required fields may stay open, because the required pass belongs to the human at completion. Stage what you have and name the gaps in the note.
...
- The literal field type strings are `text`, `textarea`, `number`, `date`, `time`, `select`, `radio`, `checkbox`, `lookup`, `record_link`, `member`, `table`, `file_upload`, plus the camelCase outlier `frameworkRequirementPicker`.
...
- A **record definition's** schema is a per-task list: each task definition (`td_*`) carries its own fields. Do not pass `--resolve-labels` when listing against an `rd_*`; the flag cannot consume a per-task schema and errors instead of relabelling.
...
3. **Referenced assets, per field type.** These fields point at other assets; resolve the reference and offer real targets:
   ...
   - `member` requires ids from the member directory (MCP: `upscaler_list({type:"members", limit:200})`; CLI: `upscaler --json list members --limit 200`). Member ids are unprefixed nanoids (only groups carry `g_`); copy them verbatim. Ask when the intended person is unclear.
...
| `member` | A member/group id string, or an array when `multiple`. On records, the `{value, label}` object form with a verified id is the reliable shape. Ask if the intended person is unclear. |
| `lookup` | `{"value": "i_*", "label": "<row title>"}`, or an array of these when `multiple`. |
| `record_link` | An array of record ids, even for one link. |
...
Writes must never carry the `upscaler:` URI prefix; that is this library's citation convention only, and the platform rejects such values with VALUE_VALIDATION_ERROR.
...
- **Record task drafts: verify with `upscaler --json get <r_*> --draft`.** Staged draft values are readable *only* through the draft read. The values-bearing list and a plain `get <r_*>` both keep reporting the record's **committed** values (usually empty, or the pre-draft state) until a human completes the task, and `get <r_*> --format schema` returns `current_value: null` for every field. Reading any of those as the verification and finding nothing is the expected result of a *successful* draft save, not a failed write. Use the plain `get <r_*>` only to confirm the task's `status` flipped to `DRAFT` and to read the display title; use `--draft` and diff `tasks[].values` against the payload you sent. Do not pass `--resolve-labels` when the definition is an `rd_*`.
- Compare table cell keys with a known-good row and expect bare column ids.
- Record values read back nested per task (`values.<taskDefinitionId>.<key>`); flatten before diffing against your payload.
