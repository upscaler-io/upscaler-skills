...
license: MIT
compatibility: Requires Upscaler MCP server (preferred) OR the `upscaler` CLI >= 0.3.0 (`pip install upscaler-cli && upscaler login`). Editor (write) permission on the target register's Organization; a non-editor token cannot even read back its own draft.
---
...
**Agent writes never finalize (HITL contract).** A create carrying values lands with status **DRAFT**, pending human review in the Upscaler app; a create with only a title stays **PENDING** until values are written. An update to a **COMPLETED** entry lands as a **pending revision**: the live values stay untouched until a human applies the revision in the Upscaler app. Never claim an entry is live, final, or completed after a write. Report a create as "saved as a draft for review" and an update to a COMPLETED entry as "proposed a revision". Pass an optional note summarizing the change for the reviewer (CLI `--note`, or `data.note` on `upscaler_manage_entry`).

Value writes reach the platform through `saveItemDraft`, which is deliberately separate from the human apply path. You therefore have **no** `appliedDraftId` and no `expectedVersion` parameter, and no way to apply or discard a draft: those are the reviewer's actions in the Upscaler app. Do not go looking for them.
...
3. **Every value write is a draft, never a finalization.** A create carrying values leaves the new entry in status DRAFT for human review (title-only creates stay PENDING, the birth state of a non-imported entry; only imports land COMPLETED). An update to a PENDING or DRAFT entry merges values and the entry stays (or becomes) DRAFT. An update to a COMPLETED entry is stashed as a pending revision; the live values and COMPLETED status are untouched until a human applies the revision in the Upscaler app, and a later draft save replaces the previous pending revision. Attach a reviewer note with each write: CLI `--note "..."` on `entry create` / `entry update`, or `data.note` in the `upscaler_manage_entry` payload. The note should summarize what was filled in or changed, so the reviewer can act on it without re-deriving your reasoning.
...
From the schema, read the full field map: fields are at `data.schema.fields[]`, each with a `key` / `label` / `type` / `required`. Every `table` field has `columns[]` (each column has its own `key` / `label` / `type`; a nested file column is `type: "file_upload"`). The literal type strings are `text`, `textarea`, `number`, `date`, `time`, `select`, `radio`, `checkbox`, `lookup`, `record_link`, `member`, `table`, `file_upload` (not `text_area` / `date_picker` / `time_picker`), plus `auto_increment`, `assessment`, `asset_picker`, `diagram`, and the camelCase outlier `frameworkRequirementPicker`. **Both** the top-level field `key`s and the nested `columns[].key`s matter.

### 2. Propose values, confirm, then build the schema-keyed payload

Load and follow the value-shape and context-derivation rules in [`../../references/form-filling.md`](../../references/form-filling.md). Then present a concise table of labels, types, proposed human-readable values, and sources **before** sending anything. Include the reviewer note you plan to send with the write. Sample, dummy, and demo requests still require confirmation — a draft save also **notifies the register definition's creator**, so a write is never invisible.

Note that unlike later draft updates, a **create carrying values must satisfy every required field**: the create path runs the required-field pass fail-closed before the entry exists (see the shared core). If a required field cannot be grounded, either derive it per the shared rules or ask, rather than sending a partial create.
...
The read-back check applies to creates and to updates of PENDING/DRAFT entries, where the values merge into the entry (status DRAFT). **An update to a COMPLETED entry is different by design:** the live values are intentionally unchanged, so an unchanged read-back is the expected outcome, not a failed write. Nothing you can read shows the stashed proposal: it lives in an editor-gated surface that only the Upscaler app reads, and the mutation response echoes the entry's **live** values, so that looks unchanged too. A call that returned without an error is the whole receipt. Report that the revision is waiting for review in the Upscaler app and stop there; do not hunt for a read that confirms it.
...
(The `entry create` response already echoes the populated `values` too — but treat `list entries` as the independent ground truth.) Cite the result as `[<entry title>](upscaler:<i_*>)` in your response, and phrase the outcome per the HITL contract: "saved as a draft for review" for a create or a draft merge, "proposed a revision (pending review in the Upscaler app)" for an update to a COMPLETED entry. Never state that the entry is live, final, or completed.

### Errors an agent may see

- `DRAFT_NOT_AVAILABLE`: you fetched a PENDING or DRAFT entry with a token that lacks editor rights. Non-editors cannot see non-finalized entries at all, and a list quietly omits them rather than erroring. If this fires on the entry you just wrote, the write probably succeeded and only the read is gated: say so and ask the user to check in the Upscaler app, rather than retrying the write.
- `VALUE_VALIDATION_ERROR`: a value failed type or shape validation. Drafts validate loosely (missing required fields are fine), so this points at a wrong shape rather than a gap: a scalar sent to a multi-valued field, a malformed date, or an `upscaler:` citation prefix leaking into an option-resolved value. (On `text`/`textarea` a leaked prefix is stored verbatim with no error, so strip prefixes before composing, not after a rejection.)

Note there is **no** `HITL_FINALIZED` for register entries. That error belongs to record tasks; an entry that a human already completed accepts your write as a pending revision instead of rejecting it.
...
- **Claiming an entry is live, final, or completed after a write.** Agent writes never finalize. A create is "saved as a draft for review"; an update to a COMPLETED entry is "proposed a revision". A human finalizes in the Upscaler app.
- **Reading an unchanged COMPLETED entry as a failed write.** An update to a COMPLETED entry stashes a pending revision and leaves live values untouched by design. The error-free call is the only receipt (the response echoes live values, and the stash has no agent-readable read); do not retry the write because `list entries` still shows the old values.
- **Skipping the reviewer note.** The note (`--note` / `data.note`) is what the reviewer sees first. Send one that summarizes the change unless the user explicitly declines.
...
- **Reaching for the reviewer's mutations.** `mergeItemValues` (with its `appliedDraftId` and `expectedVersion`), `setItemValues`, and `discardItemDraft` are the human apply/discard path in the Upscaler app. Agent value writes go through `saveItemDraft` instead, which exposes none of those parameters. If you find yourself trying to apply or discard a draft, the answer is to hand off to the reviewer, not to find another mutation.
