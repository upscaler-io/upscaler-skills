...
2. **CLI fallback:** otherwise use `upscaler asset create --type <document_definition|register_definition|record_definition|course_definition> --data @file.json` for definitions, or `upscaler entry create --definition-id <id> --data @file.json` for register entries. Confirm the session is authenticated via `upscaler status`. Show the user your proposed payload and get explicit confirmation before any write, then preview with `--dry-run` where supported (`asset`/`entry` `{create,update,delete}`). Your own propose-then-confirm is mandatory: the CLI never prompts on create/update (only `delete` has a confirm prompt, and even that is skipped under `--json` or when stdout is not a TTY), so a write runs immediately once issued.
...
- **Enforced — the create/update is rejected.** On every markdown write the backend converts to Slate and runs `validateBlocks`; any issue throws `Content validation failed`. This covers: missing/empty `name` **or `title`** on any field (exceptions: `form-auto-increment` needs `name` plus its counter options, which the markdown deserialiser fills with defaults, and `form-asset-picker` validates nothing); `form-select` / `form-radio` `values` that do not parse to a non-empty JSON array (double-quoting the JSON truncates the attribute, so the single-quote rule is enforced indirectly); `form-checkbox` values that are not all plain strings; malformed `form-assessment` `questions` (a stray apostrophe collapses the attribute to a string and the write is rejected). Validation issues of **any** severity abort the write — a "warning" (e.g. a `form-select` missing `title`) rejects just like an error. Exception: course lesson bodies are converted without validation at write time; their errors surface later, at release publish, as "Can not add asset with errors".
- **Silent degradation — the write succeeds but the data misbehaves.** Nested lists are **stored correctly on write** but the markdown **read-back** flattens them, merging each parent bullet with its children into one corrupted line — so a read-modify-write cycle destroys nesting even though a greenfield write does not; keep lists flat whenever a round-trip is expected (see `03-document-authoring.md` → "Updating an existing document"); `ff_` ids with characters outside `[A-Za-z0-9_]` (e.g. hyphens) are dropped by the read-side `select_values` projection; unknown `<form-*>` / `<format-*>` tag names are dropped at parse time; hand-authored file/image URLs do not survive; authored `fileList` entries are stripped to `uid`/`name`/`type`/`size`/`addedAt`.
...
- **Field IDs use `ff_` + 28 base62 chars from `[A-Za-z0-9]`** (no underscore, no hyphen). Generate them with `scripts/generate_field_id.py` (see below). This format and the uniqueness rule are self-enforced authoring rules: the platform's option validator checks `name` (and `title`) for non-emptiness but not the id's shape, and an id with characters outside `[A-Za-z0-9_]` (e.g. a hyphen) is silently dropped by the `select_values` projection, so the field stops resolving even though create succeeds. Never hand-craft. Every field ID is unique across the entire asset.
- **Quote every HTML attribute value as a string:** `required="true"`. (House style, not a parser rule — the parser also accepts unquoted values and turns a bare `required` into `true` — but consistent quoting is what keeps the JSON-carrying attributes safe, where quoting genuinely decides whether the write survives.)
...
- Using `required=true` or bare `required` — the parser tolerates both, but house style quotes every attribute so the JSON-carrying ones stay safe.
