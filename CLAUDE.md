...
Advanced workflow skills live in a sibling repository, `upscaler-adv-skills` (https://github.com/upscaler-io/upscaler-adv-skills), which is under internal testing. Nothing in this repo may depend on those skills being installed.
...
Five further spokes ship in `upscaler-adv-skills`: `upscaler-prep-evidence`, `upscaler-review-design`, `upscaler-setup-framework`, `upscaler-prep-management-review`, and `upscaler-report-status`. They are **optional**. `skills/upscaler-ask/SKILL.md` handles them in its "Workflows outside this library" section: if the skill is loaded, route to it; if not, answer the read-only part of the request and stop rather than half-building the artifact. The advanced repo ships an `upscaler-adv-routing` overlay skill that restores full routing when installed, so **this repo never needs editing when the advanced set changes**.
...
- `references/` — **repo-root shared references** loaded on demand by every skill via relative path (`../../references/<file>.md`). Lives outside `skills/` because the validator iterates every directory under `skills/` and expects a `SKILL.md` in each; non-skill folders there would fail validation.
  - `references/upscaler-access.md` — MCP → CLI → setup connection priority, tool/command mapping, verbatim setup message. **Duplicated byte-for-byte in `upscaler-adv-skills`**; any change here must be mirrored there. Update whenever the platform's tool surface changes.
  - `references/personas.md` — three target personas (compliance manager, engineer/PM, author) and how each skill calibrates output. Compliance manager is the primary persona; auditor is **not** a target persona. Also duplicated in `upscaler-adv-skills`, but the "skills they invoke" lines are deliberately repo-local.
  - `references/form-filling.md` — the form-filling core shared by `upscaler-write-entry` (register entries) and `upscaler-run-record` (record tasks): schema-first key resolution, per-type value shapes, deriving values from the parent asset and its referenced assets, read-back tolerances. Core-only; not duplicated.
...
- `llms.txt` — `llms.txt` index for LLM discovery. Lists every skill, the shared references, install routes, the sibling advanced repo, and the API tiers. Update alongside any rename, new skill, or breaking change.
...
# Confirm the duplicated shared reference has not drifted from the advanced repo
...
- **Never assume an advanced skill is installed.** If a workflow belongs to `upscaler-adv-skills`, the correct behaviour is to answer the read-only part and say the workflow ships separately. Never fabricate the output of a skill that is not present.
...
5. If the change touches the platform connection pattern (new MCP tools, new CLI commands, new auth flow), update `references/upscaler-access.md` **and mirror it into `upscaler-adv-skills/references/upscaler-access.md`** in the same cycle. `diff` the two before landing.
